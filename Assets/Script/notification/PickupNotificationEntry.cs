...
public class PickupNotificationEntry : MonoBehaviour
{
    ...
    private int currentAmount = 0; // Kolik celkem pøedmìtù se sebralo
    private int messageCount = 1;  // Kolikrát se ukázal error (napø. plný inv)
    ...
    // --- LOGIKA PRO SEBRÁNÍ PØEDMÌTU ---
    public void SetupPickup(Sprite icon, string itemName, int amount)
    {
        isItemPickup = true;
        baseMessage = itemName; // Uložíme si èisté jméno (napø. "Wood")
        currentAmount = amount; // První sebraný poèet
    ...
    {
        currentAmount += amount; // Pøièteme novì sebrané kusy ke starým
        UpdateText(false);
        ResetTimer();
    }
